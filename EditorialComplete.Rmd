---
title: \normalsize A New Inference about Adjuvants and Postoperative Pain Dosing?
...
Nathan L Pace, MD., MStat., Department of Anesthesiology, Univeristy of Utah, Salt Lake City, UT
...
Doleman et al.[@Doleman_inPress2017] found 25 RCTs investigating intravenous acetaminophen for postoperative pain control. In Figure 1, a classical forest plot is shown, ordered by surgical interventions ^[The 95% confidence intervals provided by Doleman et al.[@Doleman_inPress2017] were  used to estimate the standard error.[@HigginsDeek2011] The figure was generated with the Cochrane Collaboration software RevMan[@RevMan_software].] The 25 RCTs have different effect sizes; the results are inconsistent, varying widely even within the same surgery.

In Figure 2, a schematic of the acetaminophen subplot of Doleman's Figure 2[@Doleman_inPress2017] is rendered.^[by the statistical software R[@R_software] with data provided by the authors.[@Doleman_inPress2017]] Each study is represented by a colored icon. If surgery were the best way to explain why studies yield different results, then studies investigating the same surgical procedure should have similar results. Hence the same color icons should be grouped together (along the same value on the y-axis indicating the estimated effect). But studies in the same surgical group are far apart on the y-axis. By contrast, studies with similar baseline risk (close on the x-axis, i.e. baseline morphine consumption in the control group) tend to have similar effects, (they are close on the y-axis, i.e. mean morphine-equivalent reduction), illustrated in a neat alignment along the regression line in Figure 2. The conclusion is that a meta-regression controlling for baseline morphine consumption in the control group is superior to stratification by surgery in explaining between study variance in results.
